...
class player(pg.sprite.Sprite):
    ...
    def swing(self):
        if self.lxdir=='right':
            self.sword=sword(self.rect.centerx+self.rect.width/2,self.rect.centery,1)
    ...
    def isHit(self):
        for sprite in self.attacks:
            if sprite.rect.colliderect(self.rect):
                ...
                sprite.kill()
                for i in range(10):
                    self.x+=sprite.vec.x*3
                    self.y+=sprite.vec.y*3
                    self.rect.topleft=(self.x,self.y)
    def isDead(self):
        if self.HP<=0:
            self.kill()
...
P1=player(kidle[0],px,py,rGroup,screen)
...
while run:
    ...
    for event in pg.event.get():
        if event.type==pg.QUIT:
            run=False
        if event.type==pg.MOUSEBUTTONDOWN and reload==False:
            bGroup.add(P1.swing())
            reload=True
            rNum=150
    if reload==True:
